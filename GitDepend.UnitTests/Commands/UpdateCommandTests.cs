        ...
        public void Execute_ReturnsPossibleUpdates_WhenUpdatesAreNeeded_Dry()
        {
            ...
            algorithm.Arrange(a => a.TraverseDependencies(Arg.IsAny<IVisitor>(), Arg.AnyString))
                .DoInstead((IVisitor visitor, string directory) =>
                {
                    if (visitor is VerifyCorrectBranchVisitor)
                    {
                        ((VerifyCorrectBranchVisitor)visitor).Changes.Add("Would change the branch file in repo1 from branch b1 to branch b2");
                        ((VerifyCorrectBranchVisitor)visitor).Changes.Add("Would change the config file in repo1 for dependency dep1 from branch b1 to branch b2");
                    }
